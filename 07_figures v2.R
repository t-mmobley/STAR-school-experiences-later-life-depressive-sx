...
fig_res <- readRDS(file = paste0("C:/Users/tmobley/Box/STAR_School_seg_cared/",
                              "Output/results/fig_res_gee_v2.rds"))
...
fig_res$label <- 
  ifelse(grepl("black",fig_res$term) & fig_res$model=="m2",
         ...
         ifelse(grepl("black",fig_res$term) & fig_res$model=="m4",
                "No caring adult at school\n Mostly Black students\n (yes vs no)",
...
fig_res$label <- 
  factor(fig_res$label,      # Reordering group factor levels
         levels = c("Caring adult at school\n Mostly Black students\n (yes vs no)",
                    "No caring adult at school\n Mostly Black students\n (yes vs no)",
                    "Mostly Black students\n (yes vs no)"))
...
ggsave(paste0("C:/Users/tmobley/Box/STAR_School_seg_cared/",
       "Output/results/fig1_v2.tiff"), width = 7, height = 8, unit = c("in"),
       dpi = 300)
ggsave(paste0("C:/Users/tmobley/Box/STAR_School_seg_cared/",
              "Output/results/fig1_v2.jpg"), width = 7, height = 8, unit = c("in"),
       dpi = 300)
