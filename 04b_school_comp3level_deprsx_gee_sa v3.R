...
# R1: TMM adding maternal and paternal edu to fully-adjusted models
# R1: TMM removing model that include "cared" variables
for(i in c(1,6,9,12)){
  if(!exists("model_matrix")){
    model_matrix <-
      tibble("grade" = rep(c(paste0(i)), 2),
             "model_name" = c("m1", "m2"), 
             "model" = 
               c(paste0("deprsx_tscore1sd_s ~ agewav65_s + female + ",
                        "w2_students_3cat_",i),
                 paste0("deprsx_tscore1sd_s ~ agewav65_s + female + sbirth + ",
                        "maternal_edu8 + paternal_edu8 + w2_students_3cat_",i)))
  } else{
    temp <- 
      tibble("grade" = rep(c(paste0(i)), each = 2),
             "model_name" = rep(c("m1", "m2")), 
             "model" = 
               c(paste0("deprsx_tscore1sd_s ~ agewav65_s + female + ",
                        "w2_students_3cat_",i),
                 paste0("deprsx_tscore1sd_s ~ agewav65_s + female + sbirth + ",
...
# R1: TMM removed commented out intx function code snippet
